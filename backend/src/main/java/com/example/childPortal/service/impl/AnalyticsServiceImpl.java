...
public class AnalyticsServiceImpl implements AnalyticsService {
    ...
    public List<CaseTrendDTO> getCaseTrends(String period) {
        ...
        if ("daily".equalsIgnoreCase(period)) {
            ...
            for (int i = periods - 1; i >= 0; i--) {
                LocalDateTime periodStart = now.minusWeeks(i).with(java.time.DayOfWeek.MONDAY).withHour(0).withMinute(0).withSecond(0);
            ...
            for (int i = periods - 1; i >= 0; i--) {
                LocalDateTime periodStart = now.minusMonths(i).withDayOfMonth(1).withHour(0).withMinute(0).withSecond(0);
    ...
    public ResolutionRateDTO getResolutionRates() {
        ...
        dto.setHelpRequestResolutionRate(totalHelpRequests > 0 ? 
                (double) resolvedHelpRequests / totalHelpRequests * 100 : 0.0);

        long totalEntities = totalCases + totalHelpRequests;
        long totalResolved = resolvedCases + resolvedHelpRequests;
        dto.setOverallResolutionRate(totalEntities > 0 ? 
                (double) totalResolved / totalEntities * 100 : 0.0);
    ...
    public List<LocationAnalyticsDTO> getLocationAnalytics() {
        ...
        for (Case c : allCases) {
            if (c.getLocation() != null && !c.getLocation().isEmpty()) {
                ...
                LocationAnalyticsDTO analytics = locationMap.getOrDefault(location, new LocationAnalyticsDTO());
