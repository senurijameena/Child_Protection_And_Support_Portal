...
import com.example.childPortal.model.CaseTimelineEvent.EventType; import com.example.childPortal.repository.*;
import com.example.childPortal.service.CaseTimelineService; import org.springframework.beans.factory.annotation.Autowired; import org.springframework.data.domain.PageRequest;
import org.springframework.data.domain.Pageable;
import org.springframework.data.domain.Sort; import org.springframework.stereotype.Service; import java.time.Duration;
...
public class CaseTimelineServiceImpl implements CaseTimelineService {
  ...
  public List<CaseTimelineDTO> getTimelineForHelpRequest(String helpRequestId) {
    ...
    return events.stream() .map(this::convertToDTO)
  ...
  public List<CaseTimelineDTO> getFilteredTimeline(TimelineFilterDTO filter) {
    ...
    events = caseTimelineRepository.findByCaseIdOrderByEventTimeDesc(filter.getCaseId());
      ...
      events = caseTimelineRepository.findByHelpRequestIdOrderByEventTimeDesc(filter.getHelpReques tId());
    } else {
    ...
    List<CaseTimelineEvent> filteredEvents = events.stream()
      ...
      .filter(event -> {
        if (filter.getPerformedByRoles() != null && !filter.getPerformedByRoles().isEmpty()) {
          String eventRole = event.getPerformedByRole(); return filter.getPerformedByRoles().stream()
      ...
      .filter(event -> {
        if (filter.getSearchText() != null && !filter.getSearchText().isEmpty()) {
          String searchLower = filter.getSearchText().toLowerCase(); return event.getTitle().toLowerCase().contains(searchLower) ||
            event.getDescription().toLowerCase().contains(searchLower) ||
            event.getPerformedByName().toLowerCase().contains(searchLower); }
        return true; })
      .collect(Collectors.toList());
    filteredEvents = filteredEvents.stream() .filter(this::isMajorEvent) .collect(Collectors.toList());
  }
  return filteredEvents.stream() .map(this::convertToDTO) .collect(Collectors.toList());
}
@Override
...
public void createCaseCreatedEvent(String caseId, String reporterUserId, String reporterName) {
  Optional<Case> caseOpt = caseRepository.findById(caseId); if (caseOpt.isPresent()) {
    Case caseEntity = caseOpt.get();
    CaseTimelineDTO timelineDTO = new CaseTimelineDTO(); timelineDTO.setCaseId(caseId); timelineDTO.setEventType(EventType.CASE_CREATED);
    timelineDTO.setTitle("Case Created");
    timelineDTO.setDescription("New case reported: " + caseEntity.getCaseType()); timelineDTO.setPerformedByUserId(reporterUserId); timelineDTO.setPerformedByName(reporterName); timelineDTO.setPerformedByRole(reporterUserId != null ? "PU" : "ANONYMOUS"); timelineDTO.setNewStatus(caseEntity.getStatus()); timelineDTO.setNewPriority(caseEntity.getPriority());
    createTimelineEvent(timelineDTO); }
}
@Override
public void createStatusChangeEvent(String caseId, String changedByUserId, String changedByName,String previousStatus, String newStatus, String reason) { 
  ...
  timelineDTO.setDetails(reason); timelineDTO.setPerformedByUserId(changedByUserId); timelineDTO.setPerformedByName(changedByName); 
...
@Override
public void createAssignmentEvent(String caseId, String assignedByUserId, String assignedByName,String assignedToUserId, String assignedToName, String role) { 
  CaseTimelineDTO timelineDTO = new CaseTimelineDTO(); 
  timelineDTO.setCaseId(caseId); 
...
@Override
public void createPriorityChangeEvent(String caseId, String changedByUserId, String changedByName,String previousPriority, String newPriority, String reason) { 
  CaseTimelineDTO timelineDTO = new CaseTimelineDTO();timelineDTO.setCaseId(caseId); timelineDTO.setEventType(EventType.PRIORITY_CHANGED);
...
@Override
public void createInformationRequestEvent(String caseId, String requestedByUserId, String requestedByName,String requestedFromUserId, String requestedFromName) { 
  CaseTimelineDTO timelineDTO = new CaseTimelineDTO();
  timelineDTO.setCaseId(caseId); 
  ...
  timelineDTO.setPerformedByUserId(requestedByUserId); timelineDTO.setPerformedByName(requestedByName); timelineDTO.setPerformedByRole(getRoleForUser(requestedByUserId)); timelineDTO.setTargetUserId(requestedFromUserId); timelineDTO.setTargetName(requestedFromName); timelineDTO.setTargetRole("PU");
...
@Override
public void createEvidenceAddedEvent(String caseId, String addedByUserId, String addedByName,String evidenceUrl, String description) { 
  CaseTimelineDTO timelineDTO = new CaseTimelineDTO();
  timelineDTO.setCaseId(caseId); 
...
@Override
public void createCaseResolvedEvent(String caseId, String resolvedByUserId, String resolvedByName,String resolutionDetails) { 
  CaseTimelineDTO timelineDTO = new CaseTimelineDTO();
  timelineDTO.setCaseId(caseId); 
...
private CaseTimelineDTO convertToDTO(CaseTimelineEvent event) { 
  ...
  if (event.getAssignedFromUserId() != null) {
    Optional<User> user = userRepository.findById(event.getAssignedFromUserId()); user.ifPresent(u -> dto.setAssignedFromName(u.getFullName()));
  }
...
private String getRoleForUser(String userId) { if (userId == null) {
  return "SYSTEM"; 
}
...
private boolean isMajorEvent(CaseTimelineEvent event) { return switch (event.getEventType()) {
