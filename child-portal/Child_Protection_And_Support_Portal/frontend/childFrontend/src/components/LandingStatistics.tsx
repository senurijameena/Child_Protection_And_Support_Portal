    ...
    <Card className="h-100 border-0 shadow-lg text-center stat-card">
      <Card.Body className="p-4">
        ...
        {loading ? (
          ...
          <>
            <div className={`stat-value display-5 fw-bold mb-2 text-${color}`}>
              {formatValue(displayValue)}
            </div>
            <div className="stat-label fw-semibold text-dark mb-2" style={{ fontSize: '1.1rem' }}>
              {label}
            </div>
            {subtitle && (
              <div className="stat-subtitle text-muted small">
                {subtitle}
              </div>
            )}
          </>
  ...
  const statisticsCards = [
    {
      ...
      icon: '📊',
      color: 'primary',
      subtitle: stats?.lastUpdated ? `Updated ${new Date(stats.lastUpdated).toLocaleDateString()}` : ''
    },
    {
